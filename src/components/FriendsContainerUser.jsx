...
const FriendsContainerUser = props => {
    ...
    const shomoreAnpets=async(e)=>{
      ...
      const getnewPublication=await getProfileClientSide(`friends/?page=${friends.page+1}&limit=2&usernametwo=${friends.username}`,headers)
      setFrieds(x=>{
        return {
          ...x,
    ...
    const getMore=()=>{
      if(friends.page<friends.totalPages){
        getProfileClientSide(`friends/?page=${friends.page+1}&limit=2&usernametwo=${friends.username}`,headers).then(getnewPublication=>{
          setFrieds(x=>{
            return {
              ...x,
  ...
  return (
   ...
   {
    friends.allfriend.
    length!=0?friends.allfriend.map(x=>{
        return <Grid spacing={0.4} xl={12} lg={12} md={8}  gap={"1rem"} container direction={"row"}  justifyContent="center"
        alignItems="center" >
     ...
     
     {
            ...
            <button onClick={shomoreAnpets}  className='shwoMore'>ver mas</button>:<></>
     }
     <ModalSwoMore allImages={friends.showmArray} getNew={getMore} totalPages={friends.totalPages} username={friends.username} open={friends.showMore} close={close} ></ModalSwoMore>
