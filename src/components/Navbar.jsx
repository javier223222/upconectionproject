...
<div className='hamburguer'>
    ...
    <div>
      ...
      {
        search.resultSearch.length==0 && search == ''?<div><p>no hay resultados</p></div>:search.resultSearch.map(x=>{
          return <div key={x.iduser} className='resultSearch'>
            <Image onClick={()=>redirectan(x.username)} src={x.urlfile!=null?x.urlfile:acountCircle} alt='profile image' width={50} height={50}></Image>
            <h1 onClick={()=>redirectan(x.username)} >{x.username}</h1>
            <p onClick={()=>redirectan(x.username)} >{x.name} {x.apellidop} {x.apellidom}</p>
          </div>
        })
      }
